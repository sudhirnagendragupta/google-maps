...
| **June 24, 2026** | Build the dynamic re-planning loop, user-facing chat prompt, editable accommodation modal, and fix the empty-itinerary bug (where plans generated 0 days and 0 stops). | Encountered API `404` errors with deprecated Anthropic model strings. Noticed that the huge JSON generated for an 8-day trip was truncated when `max_tokens` was capped at `4000`, causing Claude to output empty plans `{}`. Saw FastAPI schema validation crashes when Claude swapped keys like `name` for `title` or nested coordinates differently. | Configured the backend with the flagship `claude-opus-4-8` model. Swapped the deeply nested Pydantic parameter structure in the `submit_itinerary` tool for a flat `itinerary_json` string, ensuring complete serialization without truncation. Increased max response tokens to `8000`. Added internal dictionary-level validation in the backend execution loop so that model schema errors are caught, returned to Claude as tool feedback, and self-corrected before finalization. Designed a premium glassmorphic onboarding UI and chat panel. |
